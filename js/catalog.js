...
function productCard(p) {
  const img = p.image_url || (Array.isArray(p.images) && p.images[0]) || SJ.PLACEHOLDER;
  ...
  return `
    <article class="card">
      <a class="card-media" href="producto.html?id=${encodeURIComponent(p.id)}">
        ...
        ${outOfStock ? '<span class="badge gray">Agotado</span>'
          : p.featured ? '<span class="badge">Destacado</span>' : ""}
      </a>
      <div class="card-body">
        <span class="card-brand">${SJ.esc(p.brand || "Sneakers")}</span>
...
async function initDetail() {
  ...
  document.title = `${p.name} — Sneakers Jhon`;

  const gallery = [p.image_url, ...(Array.isArray(p.images) ? p.images : [])]
    .filter(Boolean);
  const main = gallery[0] || SJ.PLACEHOLDER;
    ...
    <div class="detail-grid">
      <div>
        ...
        ${gallery.length > 1 ? `<div class="thumbs">${gallery
          .map((g, i) => `<div class="thumb ${i === 0 ? "active" : ""}" data-src="${SJ.esc(g)}">
              <img src="${SJ.esc(g)}" alt=""></div>`)
          .join("")}</div>` : ""}
      </div>

      <div class="detail">
        <p class="breadcrumb"><a href="index.html">Inicio</a> / <a href="catalogo.html">Catálogo</a> / ${SJ.esc(p.name)}</p>
        <span class="eyebrow">${SJ.esc(p.brand || "Sneakers")}</span>
        <h1>${SJ.esc(p.name)}</h1>
        <div class="price">${SJ.money(p.price)}</div>
        <p class="detail-desc">${SJ.esc(p.description || "Sin descripción disponible.")}</p>

        ${sizes.length ? `<span class="label">Tallas disponibles</span>
          <div class="sizes">${sizes.map((s) => `<button class="size-chip">${SJ.esc(s)}</button>`).join("")}</div>` : ""}
        ...
        <div class="meta-list">
          ...
          <div class="meta-row"><span>Stock</span><strong>${Number(p.stock) > 0 ? p.stock + " unidades" : "Agotado"}</strong></div>
  ...
  root.querySelectorAll(".size-chip").forEach((c) =>
    c.addEventListener("click", () => {
      root.querySelectorAll(".size-chip").forEach((x) => x.classList.remove("selected"));
  ...
  wsp.href =
    "https://wa.me/?text=" +
    encodeURIComponent(`Hola Sneakers Jhon, me interesa: ${p.name} (${SJ.money(p.price)})`);
}
